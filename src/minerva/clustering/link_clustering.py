    ...
    for n in adj:  # n is the shared node
        if len(adj[n]) > 1:
            for i, j in combinations(adj[n], 2):  # all unordered pairs of neighbors
                ...
                S = (
                    1.0 * len(inc_ns_i & inc_ns_j) / len(inc_ns_i | inc_ns_j)
                )  # Jacc similarity...
                heappush(min_heap, (1 - S, edge_pair))
    return [
        heappop(min_heap) for i in range(len(min_heap))
    ...
    for ind, n in enumerate(adj):  # n is the shared node
        ...
        if len(adj[n]) > 1:
            for i, j in combinations(adj[n], 2):  # all unordered pairs of neighbors
                ...
                ai_dot_aj = 1.0 * sum(
                    Aij[swap(i, x)] * Aij[swap(j, x)] for x in inc_ns_i & inc_ns_j
                )

                S = ai_dot_aj / (
                    n2a_sqrd[i] + n2a_sqrd[j] - ai_dot_aj
                )  # tanimoto similarity
                heappush(min_heap, (1 - S, edge_pair))
    return [
        heappop(min_heap) for i in range(len(min_heap))
...
def write_edge2cid(e2c, filename, delimiter="\t"):
    ...
    c2c = dict(
        (c, i + 1) for i, c in enumerate(sorted(list(set(e2c.values()))))
    )  # ugly...
    ...
    f, g = (
        open(filename + ".comm2edges.txt", "w"),
        open(filename + ".comm2nodes.txt", "w"),
    )
...
"""
    ...
    if threshold is not None:
        ...
        if is_weighted:
            ...
            if dendro_flag:
                edge2cid, S_max, D_max, list_D, orig_cid2edge, linkage = HLC(
                    adj, edges
                ).single_linkage(dendro_flag=dendro_flag)
        ...
        print("# D_max = %f\n# S_max = %f" % (D_max, S_max))
        write_edge2cid(
            edge2cid, "%s_maxS%f_maxD%f" % (basename, S_max, D_max), delimiter=delimiter
        )
