...
def get_company_info(ticker: str) -> dict | None:
    try:
        ...
        logging.error(
            f"`get_company_info`: Error fetching company info for {ticker}: {e}"
        )
        return None
...
def get_one_transcript(
    ...
    if response.status_code == requests.codes.ok:
        ...
        if not transcript_info or "transcript" not in transcript_info:
            logging.info(
                "`get_one_transcript`: No transcript found for %s, Y%s Q%s",
                ticker,
                year,
                quarter,
            )
        ...
        logging.info(
            "`get_one_transcript`: Found transcript for %s, Y%s Q%s",
            ticker,
            year,
            quarter,
        )
...
def get_transcripts(
    ticker: str, start_year: int, end_year: int, start_quarter: int, end_quarter: int
    ...
    for year in range(start_year, end_year + 1):
        for quarter in range(start_quarter, end_quarter + 1):
            try:
                transcript_map: dict[str, int | str] | None = get_one_transcript(
                    ticker, year, quarter
                )
                ...
                logging.error(
                    "get_transcripts: Error fetching transcript for %s " "Y%s-Q%s: %s",
                    ticker,
                    year,
                    quarter,
                    e,
