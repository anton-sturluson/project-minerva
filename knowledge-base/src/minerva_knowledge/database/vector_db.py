...
class MilvusVectorDB:
    ...
    def add_documents(
        self, documents: list[str], metadata: list[dict], embedding_fn: Callable
    ):
        """Add documents to the vector database."""
        if len(documents) != len(metadata):
            raise ValueError(
                "`add_documents` requires the number of documents "
                "and metadata to be the same."
            )

        vectors: list[list[float]] = embedding_fn(documents)
        data = [{"vector": vectors[i], **metadata[i]} for i in range(len(vectors))]
