...
Select every material development supported by the article summaries in this batch. Read each summary; do not select from titles or URLs alone. `portfolio_watchlist` lists the only companies eligible for that section. Do not impose a quota. A later synthesis pass will group related selections across batches.
...
{"selected_developments":[{"section":"portfolio_watchlist|worth_knowing","takeaway":"concise factual investor takeaway","article_keys":["exact-key-from-input"],"urls":["source URL from input"],"materiality":"why this could change long-term value or risk"}]}
