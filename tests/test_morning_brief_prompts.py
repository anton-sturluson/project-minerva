...
def test_collector_uses_luna_with_xhigh_reasoning() -> None:
    coordinator = _read_prompt("morning_brief_coordinator.md")

    assert "Crawler model: `openai/gpt-5.6-luna`" in coordinator
    assert "Crawler thinking: `xhigh`" in coordinator
    assert "fireworks/accounts/fireworks/routers/glm-5p2-fast" not in coordinator
